...
def get_excel(ControlID):
    ...
    fp = open(ControlID + ".xls", "wb")
    fp.write(result.content)
    fp.close()

# 讀取Excel
def excel_read(xls_path):
    workbook = xlrd.open_workbook(xls_path + ".xls")
...
if __name__ == '__main__':
    ...
    file_name = (start_date + "~" + end_date).replace("/", "-") + "-" + time.strftime("%H%M%S",time.localtime()) + ".xls"
    ...
    start = time.time() #開始def時間
    get_excel(ControlID) #下載EXCEL def
    time_count("下載時間-", start,time.time()) #計算def時間

    start = time.time() #開始def時間
    param = excel_read(ControlID) #讀取下載EXCEL def
